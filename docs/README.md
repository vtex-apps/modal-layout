...
| `shouldCompareProductContext` | `boolean` | Whether the `modal-trigger` block should compare data between the `addToCart` event and the page's Product context in order to work (`true`) or not (`false`). By setting to `true`,  the block will only trigger the modal when the `skuId`'s value (from the `addToCart`'s `cartItems` field) matches the `selectedItem`'s (from the Product context). | `false` | 
